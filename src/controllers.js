...
const controller = (req, res) => {
    try {
        ...
        const oddNumbers = inputArray.filter(x => /^\d+$/.test(x) && parseInt(x) % 2 !== 0);
        const evenNumbers = inputArray.filter(x => /^\d+$/.test(x) && parseInt(x) % 2 === 0);
        const alphabets = inputArray.filter(x => /^[a-zA-Z]$/.test(x));
        const specialChars = inputArray.filter(x => /[^a-zA-Z0-9]/.test(x));

        const sumOfNumbers = inputArray.reduce(
        (acc, x) => (/^\d+$/.test(x) ? acc + parseInt(x) : acc),
        0
        ).toString();

        let reversedConcat = alphabets.reverse().join("");
        let alternatingCaps = "";
        for (let i = 0; i < reversedConcat.length; i++) {
        alternatingCaps += i % 2 === 0
